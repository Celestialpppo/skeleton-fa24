...
public class GuitarString {
    ...
    public void tic() {
        ...
        double first = buffer.removeFirst();     // 拿出第一个 sample
        double second = buffer.get(1);           // 看第二个 sample（但不移除）
        double newSample = DECAY * 0.5 * (first + second); // 平均后衰减
        buffer.addLast(newSample);               // 加到队尾
    }
